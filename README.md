...
| 模型负责 | 引擎负责 |
| --- | --- |
| 规划当前回合是人生背景还是路线场景 | 判断当前允许进入哪些回合类型 |
| 按需选取社会力量与局内对象作为本轮素材 | 维护所选 IF 路线的三幕结构与五步叙事节拍 |
| 生成正文、抉择和叙事层面的状态变化 | 校验工具协议并结算属性、事实与关系 |
| 判断当前经历是否已经完成这一拍的语义职责 | 决定节拍迁移、生存危机和结局资格 |
| 整理已提交经历的长期摘要 | 保存原子回合、分支与匿名存档 |

模型拥有创作空间，但不能直接写入任意数值、跳过节拍状态机，或自行决定游戏结束。基础世界负责共同世界观，玩家选定的 IF 子世界包负责一生的路线承诺；引擎不预写具体案件、NPC 身份和每一幕必然发生的场景。

## 新版本具备什么

- **基础世界 + IF 路线**：基础世界统一提供时代、阵营、生活和规则；玩家再选择一条可独立增删的 IF 人生路线。路线明确长期目标和三幕问题，但不固定每场事件。
- **本局故事前提**：身世生成后，模型依据玩家人设、最终天赋、身世、世界核心和所选路线建立只属于本局的人物锚点与核心张力，避免同一路线把不同人设写成同一个人。
- **分层上下文编排**：人物、人设、天赋、本局前提、世界幕、社会力量、Lore、事实、人物档案、地点、本领、长期摘要、近期消息和当前任务统一进入 Context Orchestrator，按来源去重并按任务分配预算。
- **情景世界卡**：世界包的稳定规则与时代切片常驻；制度、地域、阵营、知名人物和冲突素材拆成独立 World Card，按当前叙事关键词、幕／拍、IF 路线偏好和局内对象激活。有限 Sticky 与随后的 Cooldown 保持连续性而不让同一线索无限自我续期。
- **两级叙事规划**：Horizon 维护当前路线阶段数个回合值得发展的张力；Turn Planner 每回合决定背景或场景、相关社会力量、关注对象、呈现方式和时间请求，不再临时抽取另一条故事形态。
...
- 选择基础世界和一条 IF 人生路线，分配五维属性、填写人物设定并选择 1 至 3 张天赋卡后，模型生成可折叠阅读的身世。
...
- 基础世界提供社会力量与按需资料；所选 IF 路线提供三幕长期目标。模型围绕这条路线生成不同人物、地点、事件与结果，而不是每回合在互不相干的故事形态之间抽卡。
...
仓库内置古代权谋、奇幻世界和现代都市三套可游玩的 v10“基础世界 + IF 子世界包”。每个世界现有六条人生路线，新增路线仍由目录自动发现，不在引擎中绑定数量。

| 世界 | 当前状态 | 世界底座／路线 |
| --- | --- | --- |
| 古代权谋 | 可游玩 | 宗族、官府、皇权、商路、军镇、乡里与流动人群；可选布衣逐鹿、庙堂权臣、边镇将帅、商贾世家、江湖宗师、方术教门。 |
| 奇幻世界 | 可游玩 | 诡异生态、异族文明、宗门传承、城邦商路与修行大道；可选宗门问道、禁区行者、散修天路、异族盟约、仙城争衡、逆命改天。 |
| 现代都市 | 可游玩 | 家庭、教育、工作、迁移、亲密关系、公共制度和个人创造；可选学途成章、事业造物、相伴此生、城中远行、众声之间、人生重启。 |

基础世界先给出“此刻这个世界正在怎样运转”的切片；独立 World Card 保存可按需遇见的地域、组织、知名人物与矛盾素材；IF 线只串联其中相关部分并规定一生要回答的问题。具体案件、局内人物、冲突、地点和结果仍由每局上下文生成，不硬编码为固定小说情节。

### 路线如何保持开放

每条 IF 线提供从起点到结果的三个阶段，但只定义目标、戏剧问题、压力方向与阶段意义，不穷举事件。相同的“布衣逐鹿”可以由军户、商贩、地方豪强或皇族旁支进入；具体人物、地点、阵营关系、本领和选择后果来自本局人设与已经发生的事实。`payoff` 结束当前阶段，人物、关系、属性和代价继续进入下一幕，已完成的事情不会重新变成开放问题。
...
3. **Session Premise**：身世完成后，将人物锚点、核心张力与玩家已经选定的 IF 路线组合为本局前提。
4. **Horizon**：在需要短程规划时，模型围绕当前路线幕提出未来数个回合值得发展的戏剧问题、张力和阶段结果。
5. **Turn Planner**：模型决定本轮背景或场景，并按需选取社会力量、关注引用、呈现方式与时间请求，不生成玩家正文或切换人生路线。
...
| `plan_narrative_horizon` | 提出当前 IF 路线幕的短程发展意图 |
| `plan_narrative_turn` | 选择本回合类型、社会力量、关注对象与呈现方式 |
...
叙事引擎不依赖固定的路线数量、阵营数量或具体事件。新增世界时分别创作基础世界与任意数量的 IF 子世界包，不需要重写 Agent Runtime：
...
3. 复制 `world-cards.example.json`，按题材拆分后放入 `data/narratives/world-cards/<worldId>/`；多个文件会合并为该世界的按需资料库。
4. 复制 `story-pack.example.json`，创作一条三幕 IF 路线，并放入 `data/narratives/story-packs/` 下任意子目录。子包可用 `worldCardRefs` 表达召回偏好，但不复制世界卡正文。
5. 可以继续独立增删子世界包；不需要同步修改其他路线、前端枚举或代码，也不需要为每个拍点穷举静态事件。

首次运行会把基础世界列表写入 `storage/custom-content.json`。已有内容存储的安装需要同步其 `worlds` 列表；修改内容文件后需要重启后端。新局会快照所选 IF 路线，路线文件的后续修改不会改变已有存档。

详细说明见[基础世界包说明](./examples/world-pack/WORLD_PACK_GUIDE.txt)、[IF 子世界包说明](./examples/world-pack/STORY_PACK_GUIDE.txt)、[小说蒸馏与世界包指南](./docs/NOVEL_DISTILLATION_GUIDE.md)和[配置指南](./docs/CONFIG_GUIDE.md)。
...
apps/
  backend/
    ...
    src/story-packs.ts        IF 子世界包的发现与校验
    src/narrative-experience.ts 基础世界与所选路线的运行时组合
...
data/                         基础世界、独立 World Card、IF 子世界包、事件与天赋数据
examples/world-pack/          基础世界、World Card 与 IF 子世界包社区模板
storage/                      单实例运行期数据与配置
```
...
- **游戏引擎**：基础世界 + IF 子世界包、五拍状态机、事实账本、人物与资产、属性结算、生存危机、三档结局
