...
| 从世界包的路线、阵营目录中选择本轮视角 | 维护世界幕与五步叙事节拍 |
| 生成正文、抉择和叙事层面的状态变化 | 校验工具协议并结算属性、事实与关系 |
| 提出短程叙事意图与结局申请 | 决定时间推进、生存危机和结局资格 |
| 整理已提交经历的长期摘要 | 保存原子回合、分支与匿名存档 |

模型拥有创作空间，但不能直接写入任意数值、创造世界包之外的路线 ID、跳过主线状态机，或自行决定游戏结束。引擎掌握规则，但不预写具体案件、NPC 身份和每一幕必然发生的场景。

## 新版本具备什么

- **分层上下文编排**：人物、人设、天赋、世界幕、路线、阵营、Lore、事实、人物档案、地点、本领、长期摘要、近期消息和当前任务统一进入 Context Orchestrator，按来源去重并按任务分配预算。
- **两级叙事规划**：Horizon 维护当前世界幕数个回合值得发展的张力；Turn Planner 每回合重新决定背景或场景、路线、阵营、关注对象、呈现方式和时间请求。
...
- **异步长期记忆**：Memory Curator 只整理已经提交的 Episode，按整局、世界幕、路线、阵营和人物生成 Digest；通过 revision 校验写回，不覆盖更新中的存档。
...
当前版本已通过后端 64 项回归、后端 TypeScript 编译、前端生产构建和 shared `--noEmit` 检查。真实叙事质量、长局重复率、响应延迟与 Token 成本仍取决于所选模型和实际对局，不以静态测试结果代替模型实测。
...
- 路线是经历视角，不是互斥职业。模型每回合都可从当前世界包的完整路线目录重新选择。
...
仓库内置三套 v6 叙事世界包。每套世界包独立提供文风、三幕主线、阵营、开放路线、Lore、成长方向、生存规则和三档结局。

| 世界 | 三幕中心 | 体验方向 |
| --- | --- | --- |
| 古代权谋 | 案起入局 → 立身周旋 → 大局担当 | 从案件或现实裂缝进入局势，在组织与关系中立身，最终面对战争与更大责任。 |
| 奇幻世界 | 成长与奇遇 → 宗门试炼与难题 → 跨越与飞升 | 在诡异、异族和宗门竞争中成长，积累本领，经历试炼并决定道途归宿。 |
| 现代都市 | 学习成长 → 选择与迷茫 → 关系与成就 | 经历学习、升学或求职、独立生活与长期关系，婚姻和事业并非唯一答案。 |

这些内容只确定题材方向和阶段职责。具体案件、组织、人物、冲突、地点和结果由每局上下文生成，不硬编码为固定小说情节。

### 古代权谋如何保持开放

家门、仕途、商路、法度、边地与人情等经历贯穿全部世界幕。模型可在任意拍点切换经历视角，它们共同服务于当前世界幕，而不是六条互相排斥的主线：

1. **案起入局**：人物从生活和身份出发，接触案件、制度裂缝或尚未有人承担的现实矛盾。案件可以是新事，也可以带有旧线索。
2. **立身周旋**：前一幕形成的关系与代价把人物带入组织、阵营或地方网络，在新的位置面对独立矛盾。
3. **大局担当**：战争或外部危机真正进入人物与社会生活；人物依据已有身份和能力参与交锋、组织、保护或求存。

每幕 payoff 只结束当前幕的中心矛盾。人物、关系、属性、代价和历史选择继续进入下一幕，已完成的事情不会重新变成开放问题。
...
3. **Horizon**：在需要主线规划时，模型提出当前幕未来数个回合值得发展的戏剧问题、张力和阶段结果。Horizon 不选择、排除或锁定路线。
4. **Turn Planner**：模型从世界包完整目录选择本轮背景或场景、路线、阵营、关注引用、呈现方式与时间请求，不生成玩家正文。
5. **Precise Recall + Renderer**：引擎依据计划重新召回相关人物、事实、地点、本领和 Lore，只开放与计划匹配的一个 Renderer 工具。
6. **Prose Review**：主线场景、抉择结果和结局正文进行连贯性、重复与内部结构泄露检查；结构化游戏结果不交给审校器修改。
7. **Engine Settlement**：规则引擎结算属性、事实、关系、资产、时间、抉择和生存状态。任何失败都不会发布半成品回合。
8. **Commit**：成功结果写入 `Episode`、`TurnRecord`、事实账本和存档；payoff 同步写入 `Act Canon`。
9. **Memory Curator**：保存后异步整理 4 至 6 个尚未覆盖的 Episode；payoff 和 ending 可提前触发。摘要通过 revision 和 Episode 覆盖范围校验后提交。
...
| 工具 | 作用 |
| --- | --- |
| `plan_narrative_horizon` | 提出当前世界幕的短程发展意图，不选择路线 |
| `plan_narrative_turn` | 选择本回合类型、路线、阵营、关注对象与呈现方式 |
...
- 主线场景：Planner、Renderer、正文整理；必要时额外生成 Horizon。
- 抉择结算：后果生成与正文整理。
...
叙事引擎不依赖古代世界的路线数量、名称和具体剧情。新增世界时主要补充世界数据，而不需要重写 Agent Runtime：
...
3. 在叙事包中配置文风、三幕职责、阵营、开放路线、Lore、成长方向、生存规则和好/普通/坏结局蓝图。
4. 不需要为每条路线、每个拍点穷举静态事件；人物、地点、本领和动态事实可在对局中生成并持久化。
...
- **Agent 能力**：function calling、Horizon、Turn Planner、任务级上下文、Renderer、Prose Review、Memory Curator
- **游戏引擎**：数据驱动世界包、五拍状态机、事实账本、人物与资产、属性结算、生存危机、三档结局
- **传输与存储**：NDJSON 流式回合、匿名会话、文件存档和分支恢复

## 当前边界与后续方向

当前版本不依赖 SillyTavern，也没有直接复制其运行时；项目借鉴了沉浸式文本工具按任务激活世界信息、分层上下文和长期摘要的组织思想。当前未使用向量 RAG，也不允许模型连接容器外工具。
